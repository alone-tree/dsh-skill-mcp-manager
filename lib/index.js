...
async function apply(ctx, config = {}) {
  ...
  const uiHandlers = {
    ...
    async getSessionAudit() {
      // This request IS the scan trigger (see `auditRunner` above), and it must
      // not be awaited: the scan takes tens of seconds and this response sits on
      // the client's page-load path. The client polls `pending` instead.
      void auditRunner.kick();
      const record = await auditRunner.read();
      // No record yet means the scan has not landed; the client keeps asking
      // rather than assuming there is nothing to report.
