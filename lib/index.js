...
// Resolved lazily (not at module load) so tests and embedders can point
// DSH_HOME at a scratch home before calling apply() — a module-load read made
// every test run write into the user's real ~/.dsh.
function dshHome() {
...
const Config = z.object({
  ...
  // Ask the user (through the host approval seam) before a MODEL-driven
  // mcp_register spawns a new or connection-changed stdio command.
  confirmStdioRegister: z.boolean().default(true),
  // Extra `host` or `host:port` values accepted for the browser routes when
  // the GUI is reached through a LAN hostname (default policy allows only
  // localhost and IP-literal Host headers).
...
function resolveDataDir(config) {
  const raw = config.dataDir && config.dataDir.trim() !== "" ? config.dataDir : join(dshHome(), "skill-mcp-manager");
...
// Mirror the host subprocess seam's scrub (dsh-subprocess, also used by the
// native dsh-mcp-client): credential-shaped names and stale `DSH_*` keys are
// dropped from the ambient parent env before it reaches a spawned MCP server.
// Explicit entry env merges AFTER the scrub, so a deliberately supplied value
// always survives.
...
// Documented contract: an env value may be a literal or a `$VAR` reference
// into the host environment. Expand references at spawn time so secrets never
// need to be persisted in registry.json; an unset reference expands to "".
function expandEnvReference(value) {
  if (typeof value !== "string") return value;
  const match = /^\$([A-Za-z_][A-Za-z0-9_]*)$/.exec(value);
  return match === null ? value : process.env[match[1]] ?? "";
}

function buildChildEnv(extra) {
  const env = scrubbedParentEnv();
  for (const [key, value] of Object.entries(extra ?? {})) env[key] = expandEnvReference(value);
  return env;
}
...
async function apply(ctx, config = {}) {
  ...
  // How long a session waits before retrying an eager MCP whose connection
  // failed. Without this, a down eager server re-attempts (and blocks the
  // step for trialTimeoutMs) on EVERY agent step.
  const EAGER_RETRY_COOLDOWN_MS = 60000;

  async function ensureEager(session, entry) {
    // Already connected and registered against the current entry state —
    // nothing to do. The pre-step hook calls this on every step; disposing
    // and re-registering the native tools each time would flap the session's
    // tool list. entry.lastSyncAt changes on mcp_register / setTier /
    // setToolEnabled, and a reload clears the token via disposeSessionEntry.
    const token = `${entry.lastSyncAt ?? ""}`;
    const current = session.eagerTokens.get(entry.name);
    if (
      current !== undefined &&
      current.token === token &&
      session.connections.get(entry.name) !== undefined &&
      session.nativeDisposers.get(entry.name) !== undefined
    ...
    session.eagerTokens.set(entry.name, { token, count: disposers.size });
  ...
  async function disposeSessionEntry(session, entryName) {
    ...
    session.eagerTokens.delete(entryName);
  ...
  async function ensureSessionRuntime(agent) {
    ...
    for (const entry of registry.entries) {
      if (entry.tier === "eager") {
        // Fast path: already registered and unchanged (most steps).
        if (session.eagerTokens.has(entry.name) && session.connections.has(entry.name)) continue;
        const failedAt = session.eagerFailures.get(entry.name);
        if (failedAt !== undefined && Date.now() - failedAt < EAGER_RETRY_COOLDOWN_MS) continue;
        try {
          await ensureEager(session, entry);
          session.eagerFailures.delete(entry.name);
        } catch (error) {
          session.eagerFailures.set(entry.name, Date.now());
  ...
  // Write the patch file behind a timestamped backup, then prune old backups
  // so the profile directory (and the secrets inside those backups) does not
  // accumulate unbounded copies.
  const PATCH_BACKUPS_KEPT = 5;

  async function writePatchWithBackup(patchPath, next) {
    ...
    try {
      ...
      const names = (await readdir(dir)).filter((candidate) => candidate.startsWith(prefix)).sort();
      for (const stale of names.slice(0, Math.max(0, names.length - PATCH_BACKUPS_KEPT))) {
  ...
  async function reconcile() {
    const patchPath = join(dshHome(), "profiles", resolveProfile(config), "cordis.patch.yml");
    let current;
    try {
    ...
    if (next !== current || imported > 0) {
      if (next !== current) {
        await writePatchWithBackup(patchPath, next);
  ...
  if (typeof ctx.inject === "function") {
    ctx.inject(["webServer"], (hostCtx) => {
      mountUi(hostCtx, uiHandlers, { dataDir, webServer: hostCtx.webServer, trustedOrigins: config.trustedOrigins ?? [] });
    });
  } else {
    mountUi(ctx, uiHandlers, { dataDir, trustedOrigins: config.trustedOrigins ?? [] });
  }

  // ── confirmation gate for model-driven stdio spawns ──
  // mcp_register is model-facing: without a gate, any session (including a
  // prompt-injected one) can persist and immediately spawn an arbitrary local
  // command — and warmSnapshots re-spawns it at every boot. When the host has
  // an approval seam composed, ask the user before a registration that adds
  // or changes a stdio connection contract. Tier/notes-only edits, HTTP
  // transports, native imports, and UI edits never pass through this gate.
  // Hosts without an approval seam keep today's silent behavior.
  ctx.on("tools/pre-execute", async (exec, next) => {
    const args = exec?.arguments;
    if (
      config.confirmStdioRegister !== false &&
      exec?.name === "mcp_register" &&
      args !== null && typeof args === "object" &&
      ctx.get("approval") !== undefined
    ) {
      const existing = registry.entries.find((entry) => entry.name === args.name);
      const transport = args.transport ?? existing?.transport;
      const spawnChanges = transport === "stdio" && (
        existing === undefined ||
        args.command !== undefined ||
        args.args !== undefined ||
        args.env !== undefined ||
      ...
      if (spawnChanges) {
        const command = args.command ?? existing?.command ?? "";
        const argv = Array.isArray(args.args) ? args.args : existing?.args ?? [];
        const tier = args.tier ?? existing?.tier ?? "on-demand";
        return {
          kind: "ask",
          reason: `mcp_register "${args.name}" (tier=${tier}) will spawn a local process now and at every future boot: ${command} ${argv.join(" ")}`.trim(),
        };
      }
    }
    return next();
  ...
  ctx.on("agent/pre-step", async ({ agent, signal }, next) => {
    ...
    const entries = catalogEntries(visible);
    ...
    const message = renderCatalog(entries, digest);
    return { kind: "enter", messages: [...decision.messages, message] };
  });

  // Entries are the projection the model-facing prose and the digest both
  // derive from (same-source principle, like the built-in skill-catalog); the
  // digest alone is persisted in the source.
  function catalogEntries(visible) {
  ...
  function catalogDigest(entries) {
    const canonical = entries
      .map((entry) => JSON.stringify([entry.name, entry.tier, entry.description, entry.notes, entry.tools.map((tool) => [tool.name, tool.description])]))
      .join("\n");
  ...
  // The digest written into the source is the durable projection of the
  // catalog (the host round-trips `source` verbatim — verified on-machine).
  // Records without a readable digest (older formats, hand-edited logs) count
  // as "not this plugin's catalog" and never as a visible catalog.
  ...
  // digest of the newest catalog still visible on the session surface.
  ...
  function renderCatalog(entries, digest) {
    const lines = [];
    for (const entry of entries) {
    ...
    const text = [
      ...
      "</mcp_catalog>",
      "",
      "Usage: mcp_load / mcp_register / mcp_call are this capability library's built-in management tools — call them directly; they belong to no MCP server, so never route them through mcp_call. Eager servers expose mcp__<server>__<tool> directly. For on-demand servers, call mcp_load first (tool names + parameter schemas), then mcp_call {name, tool, args}; calling mcp_call before mcp_load errors. Entries with tools have a cached snapshot; entries without tools need mcp_load first. Disabled servers and disabled tools are hidden.",
      "</system-reminder>",
    ].join("\n");
    return createUserMessage({
      content: [{ type: "text", text }],
      // No declared `form`: the host renders context without a recognized form
      // as the model-facing text itself (OpaqueBody), so what a human expands
      // is exactly what the model read. The source stays minimal ({kind,
      // digest}): SourceFields displays every source field, so carrying
      // entries there would render the same information twice.
