...
import { runAuditIfNeeded, readAuditRecord } from "./session-audit.js";
...
// How long after plugin load the legacy session audit is allowed to start.
// The scan is far too slow for the startup path: the desktop host fails
// `host-boot` on a ~120s watchdog, so anything of that size must wait until the
// boot stage is over.
const SESSION_AUDIT_DELAY_MS = 8000;
...
async function apply(ctx, config = {}) {
  ...
  const uiHandlers = {
    ...
    async getSessionAudit() {
      const record = await readAuditRecord(dataDir);
      // No record yet means this start's scan has not landed; the client keeps
      // asking rather than assuming there is nothing to report.
      if (record === null) return { pending: true };
      return {
        pending: false,
  ...
  // session log.
  //
  // It MUST NOT be awaited in the startup path. A full scan of a large library
  // runs 46-136s on this machine, and the desktop host fails `host-boot` on a
  // ~120s watchdog — awaiting it here put DSH into safe mode on every restart
  // (2026-09-14). Deferred, it costs nothing at boot: the client already polls
  // `pending` until the record lands, and on every later start the previous
  // record is already on disk, so the notice is simply there when the page
  // opens.
  //
  // It re-runs on every start while its `check` flag stays set; a clean scan
  // retires the flag by itself. There is no dismiss action, so an accidental
  // click can never hide a real problem.
  //
  // Skipped for the `__test__` sentinel profile: tests point the plugin at a
  // throwaway dataDir precisely so they never touch the real home, and reading
  // the real sessions tree would break that isolation — and make every test that
  // calls apply() wait for a full scan of it.
  if (resolveProfile(config) !== "__test__") {
    const auditTimer = setTimeout(() => {
      runAuditIfNeeded(dataDir, join(DSH_HOME, "sessions"), ctx.logger).catch((error) => {
        ctx.logger.warn(`skill-mcp-manager: session audit failed: ${String(error?.message ?? error)}`);
      });
    }, SESSION_AUDIT_DELAY_MS);
    // Never hold the process open, or the boot stage, for a background check.
    if (typeof auditTimer.unref === "function") auditTimer.unref();
    ctx.effect(() => () => clearTimeout(auditTimer));
  }
