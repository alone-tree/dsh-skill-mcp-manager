...
async function apply(ctx, config = {}) {
  ...
  // Runs in the startup path, before the slow steps below, so that a first
  // install has its answer on screen when the page opens instead of surfacing
  // minutes into the session. Sessions written before 1.1.5 carry a source kind
  // the v0 -> v3 migration refuses and cannot be opened at all once DSH reaches
  // 2.0.9; this only detects them so the UI can point at the repair guide, and
  // it never writes to a session log. It re-runs on every start while its
  // `check` flag stays set — a clean scan retires the flag by itself, so the
  // startup cost is paid until the machine is either repaired or shown clean,
  // and then never again. There is no dismiss action, so an accidental click
  // can never hide a real problem.
  ...
  if (resolveProfile(config) !== "__test__") {
    try {
      await runAuditIfNeeded(dataDir, join(DSH_HOME, "sessions"), ctx.logger);
    } catch (error) {
