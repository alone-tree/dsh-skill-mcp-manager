...
async function apply(ctx, config = {}) {
  ...
  // Read-only reconcile (M0): report drift between the registry and the profile patch.
  async function reconcile() {
    ...
    for (const entry of registry.entries) {
      ...
      const shadowId = entry.systemEntryId ?? `mcp-${entry.name}`;
      const hasShadow = text.includes(`- id: ${shadowId}`) || text.includes(`id: ${shadowId}`);
      if (!hasShadow) {
        ctx.logger.warn(`skill-mcp-manager: registry entry "${entry.name}" has no shadow entry (${shadowId}) in ${patchPath} — will be written in M2`);
      } else if (!/disabled\s*:\s*true/.test(text)) {
        ctx.logger.warn(`skill-mcp-manager: shadow entry "${shadowId}" is not disabled — risk of double registration`);
      }
    }
  }
