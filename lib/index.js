...
function splitPatchAroundManaged(text) {
  ...
  if (start < 0) {
    return { prefix: text, relocated: "", suffix: "" };
  }
...
async function apply(ctx, config = {}) {
  ...
  async function importNative() {
    ...
    const { prefix: nativeText } = splitPatchAroundManaged(current);
  ...
  async function reconcile() {
    ...
    const nativeEntries = parseNativeMcpEntries(prefix);
  ...
  const mcpLoad = {
    ...
    async execute(args) {
      ...
      if (entry.tier === "eager") {
        const toolCount = await ensureEager(entry);
        return { text: `MCP "${entry.name}" reloaded (eager): ${toolCount} enabled native tool(s) re-registered.` };
      }
  ...
  const uiHandlers = {
    ...
    async setToolEnabled(name, toolName, enabled) {
      ...
      const knownTools = connection?.tools ?? entry.tools ?? [];
      ...
      entry.lastSyncAt = new Date().toISOString();
      await saveRegistry(dataDir, registry);
      if (entry.tier === "eager") await ensureEager(entry);
      return entryView(entry, false, connections.has(name));
    },
    ...
    async loadMcp(name, peek) {
      ...
      if (entry.tier === "eager") {
        const toolCount = await ensureEager(entry);
        return { ok: true, text: `MCP "${entry.name}" reloaded (eager): ${toolCount} enabled native tool(s) re-registered.` };
      }
