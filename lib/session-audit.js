...
export async function runAuditIfNeeded(dataDir, sessionsRoot, logger) {
  ...
  if (existing !== null && existing.check !== true) return existing;
 ...
 *
 * `kick` reports whether it actually started a scan, because the client has to
 * tell "a scan is running, tell the user to wait" apart from "this check is
 * already settled, stay quiet" — without that, they look identical, and a
 * machine that will never scan again would keep showing a progress notice.
 * @param dataDir - the plugin's data directory.
 * @param sessionsRoot - the sessions root.
 * @param logger - the host logger.
 * @returns `{kick, read, running}`; `kick` must NOT be awaited on a request path.
 */
export function createAuditRunner(dataDir, sessionsRoot, logger) {
  // "Busy" covers the whole run, not just the scan loop. A run begins by reading
  // the record off disk, and reporting idle during that read told the client the
  // answer had landed when it had not: the client stopped asking, so a scan that
  // finished afterwards was never reported (found on-machine 2026-09-14 — the
  // wait notice appeared, the result never did).
  let busy = false;
  let work = null;
  return {
    kick() {
