...
export function createAuditRunner(dataDir, sessionsRoot, logger) {
  // Tracks the scan itself, not the whole promise: `running()` is what the UI
  // shows a wait notice for, and it must go false the moment the answer lands.
  let scanning = null;
  return {
    kick() {
      if (scanning === null) {
        scanning = runAuditIfNeeded(dataDir, sessionsRoot, logger)
          .then((record) => {
            scanning = null;
            return record;
          })
    ...
    /**
     * Wait for the in-flight scan, if there is one. Off the request path only.
     * @returns the scan result, or null when nothing was in flight.
     */
    async wait() {
      return scanning === null ? null : await scanning;
    },
  };
}
