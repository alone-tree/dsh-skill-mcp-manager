 ...
 *
 * Bumped to 2 when the gate became a persisted `check` boolean: a version-1
 * record has no `check` member at all, and reading that as "the flag is set"
 * would rescan a machine whose previous answer was already delivered. An
 * unrecognised record is ignored instead, so the audit simply runs once more.
 */
 ...
 * yield 135 KB total, one header line each, instead of 917 MB).
...
export async function readAuditRecord(dataDir) {
  try {
    ...
    if (record.schema !== 1 && record.schema !== AUDIT_SCHEMA) return null;
    if (typeof record.check === "boolean") return record;
    // Schema 1 carried `dismissedAt` instead of `check`. Migrate that decision
    // rather than dropping the record: discarding it makes the next run rescan,
    // and a scan heavy enough to be killed mid-flight records nothing, so the
    // machine would rescan on every start forever (exactly what 2026-09-14's
    // schema bump caused).
    return {
      ...record,
      check: !(typeof record.dismissedAt === "string" && record.dismissedAt !== ""),
    };
  } catch {
    return null;
  }
 ...
 * @param onScan - called when the scan itself begins, which is later than the
 *   call: the record is read first, and a retired check stops before any scan.
 * @returns `{record, scanned}` — the record that now applies (null when none
 *   exists yet), and whether this call actually scanned. Callers need the second
 *   one to tell "an answer we just produced" from "an answer read off disk".
 */
export async function runAuditIfNeeded(dataDir, sessionsRoot, logger, onScan) {
  const existing = await readAuditRecord(dataDir);
  // Only an explicit `true` means "keep checking". Anything else — a retired
  // flag, or a migrated schema-1 record whose answer was already delivered —
  // means there is nothing left to do here.
  if (existing !== null && existing.check !== true) return { record: existing, scanned: false };
...
/**
 * Build the guard that keeps the scan off the startup path.
 *
 * The scan cannot be scheduled from `apply()` at all. It is heavy enough to
 * starve the host's boot work — one frame can block the event loop for over a
 * second, and a large library takes 30-136s in total — while the desktop host
 * fails `host-boot` on a ~120s watchdog and drops into safe mode. Both a direct
 * `await` and an 8s `setTimeout` still landed inside that window (2026-09-14,
 * twice), because plugin load happens early in boot and nothing scheduled from
 * there can know when boot ends.
 *
 * So the trigger is the first UI request instead: if the page is asking, the app
 * is up. The scan then runs in the background while the client polls; a machine
 * whose answer is already on disk never scans at all.
 *
 * `kick` only reports that work was requested; whether a scan is running and
 * whether one ever ran are separate signals (`running`, `checked`), because the
 * client has to tell "a scan is running, tell the user to wait" apart from "this
 * check is already settled, stay quiet" — without that, they look identical, and
 * a machine that will never scan again would keep showing a progress notice.
 * @param dataDir - the plugin's data directory.
 * @param sessionsRoot - the sessions root.
 * @param logger - the host logger.
 * @returns `{kick, read, running, checked, wait}`; `kick` must NOT be awaited on
 *   a request path.
 */
export function createAuditRunner(dataDir, sessionsRoot, logger) {
  // "Running" means the scan itself is under way, not the bookkeeping that
  // precedes it: the record read takes milliseconds, and announcing a wait for
  // it made a check that will never scan flash a progress notice (found
  // on-machine 2026-09-14).
  let busy = false;
  let work = null;
  // Whether a scan has actually been started in this process, which is not the
  // same question as "is one running right now": the UI announces a result only
  // when a scan ran here, so that reading a long-settled record is not reported
  // as fresh news on every start. A retired check never scans, so it stays false.
  let ran = false;
  return {
    kick() {
      if (work === null) {
        work = runAuditIfNeeded(dataDir, sessionsRoot, logger, () => {
          busy = true;
        })
          .then((outcome) => {
            ran = outcome.scanned === true;
            busy = false;
            return outcome.record;
          })
          .catch((error) => {
            logger.warn(`skill-mcp-manager: session audit failed: ${String(error?.message ?? error)}`);
            // Let the next request retry instead of latching the failure.
            busy = false;
            return null;
          });
      }
      // Work is under way or was already settled within this call; either way the
      // caller has nothing to await on a request path.
      return { started: true };
    },
    read() {
      return readAuditRecord(dataDir);
    },
    running() {
      return busy;
    },
    ...
    /**
     * Wait for the in-flight run, if there is one. Off the request path only.
     * @returns the run result, or null when nothing was in flight.
     */
    async wait() {
      return work === null ? null : await work;
    },
  };
}
