 ...
 * is up. The scan then runs in the background while the client polls
 * `pending`; a machine whose answer is already on disk never scans at all.
 * @param dataDir - the plugin's data directory.
 * @param sessionsRoot - the sessions root.
 * @param logger - the host logger.
 * @returns `{kick, read}`; `kick` must NOT be awaited on a request path.
 */
export function createAuditRunner(dataDir, sessionsRoot, logger) {
  ...
  return {
    kick() {
      ...
      inFlight = runAuditIfNeeded(dataDir, sessionsRoot, logger).catch((error) => {
