...
export function registerRecursiveSkillProvider(ctx, dataDir) {
  let control;
  ctx.skills.registerProvider((ctrl) => {
    control = ctrl;
    return {
      name: PROVIDER_NAME,
      async list(options) {
  ...
  // Watch the configured roots and invalidate the provider on any change, so
  // added/edited/deleted skills are re-discovered without a restart.
  ctx.effect(() => {
    let disposed = false;
    let timer;
    const watchers = [];
    ...
    function scheduleInvalidate() {
      ...
      timer = setTimeout(() => {
        ...
        try {
          ...
          /* provider already released; ignore */
        }
      }, 200);
    }
    ...
    (async () => {
      let settings;
      try {
        settings = await loadSettings(dataDir);
      } catch {
        return;
      }
      const roots = Array.isArray(settings.customRecursiveDirs)
        ? settings.customRecursiveDirs.filter((root) => typeof root === "string" && root.length > 0)
        : [];
      if (disposed) return;
      for (const root of roots) {
        try {
          watchers.push(watch(root, { recursive: true }, () => scheduleInvalidate()));
        } catch {
          /* missing dir or unwatchable path — skip */
        }
      }
    })();
...
async function scanRoot(root, signal, ctx) {
  const candidates = [];
  // The configured root itself may be a skill (e.g. global-skills/SKILL.md).
  const rootFallbackName = toKebabCase(basename(root));
  const rootLocator = { path: join(root, "SKILL.md"), directory: root, fallbackName: rootFallbackName };
  const rootParsed = await readSkillFile(rootLocator.path, signal, ctx, rootFallbackName);
  if (rootParsed !== undefined) candidates.push(buildCandidate(rootParsed, rootLocator));
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      const locator = { path: join(childDir, "SKILL.md"), directory: childDir, fallbackName };
      const parsed = await readSkillFile(locator.path, signal, ctx, fallbackName);
      if (parsed !== undefined) candidates.push(buildCandidate(parsed, locator));
      await walk(childDir);
    }
  }
  await walk(root);
...
function buildCandidate(parsed, locator) {
  return {
    name: parsed.name,
    description: parsed.description,
    ...(parsed.whenToUse !== undefined ? { whenToUse: parsed.whenToUse } : {}),
...
// ── frontmatter editing (model-invocation toggle) ─────────────────────────

// Enable/disable model invocation by writing or removing the
// `disable-model-invocation` frontmatter key. The edit is line-level so the
// body and every other frontmatter line (order, comments) are preserved
// byte-for-byte; only the one key changes. Writes are atomic (tmp + rename).
//
// Semantics (aligned with dsh-skill-filesystem): `disable-model-invocation`
// only affects whether the MODEL can invoke the skill — the user's slash
// `/name` invocation is controlled separately by `user-invocable` and is
// never touched here.
export async function setDisableModelInvocation(path, disabled) {
  const raw = await readFile(path, "utf8");
  const eol = raw.includes("\r\n") ? "\r\n" : "\n";
  const lines = raw.split(/\r?\n/);
  const openIndex = lines.findIndex((line) => line.trim() === "---");
  if (openIndex < 0) throw new Error(`skill file has no frontmatter: ${path}`);
  const closeIndex = lines.findIndex((line, i) => i > openIndex && line.trim() === "---");
  if (closeIndex < 0) throw new Error(`skill frontmatter is not closed: ${path}`);
  const keyIndex = lines.findIndex(
    (line, i) => i > openIndex && i < closeIndex && /^\s*disable-model-invocation\s*:/.test(line),
  );
  if (disabled) {
    const replacement = "disable-model-invocation: true";
    if (keyIndex >= 0) lines[keyIndex] = replacement;
    else lines.splice(openIndex + 1, 0, replacement);
  } else if (keyIndex >= 0) {
    lines.splice(keyIndex, 1);
  }
  const next = lines.join(eol);
  if (next === raw) return { changed: false, path, modelInvocable: !disabled };
  const tmp = `${path}.tmp-${process.pid}`;
  await writeFile(tmp, next, "utf8");
  await rename(tmp, path);
  return { changed: true, path, modelInvocable: !disabled };
}
