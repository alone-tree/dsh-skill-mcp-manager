...
// One-off repair guide for sessions written before the 1.1.5 source-format fix.
      ...
      handler: async () => {
        ...
        const text = entries.length === 0
          ...
          : `Registered MCP servers (${entries.length}):\n${entries
              ...
              .join("\n")}`;
        const audit = await handlers.getSessionAudit();
        if (audit.pending !== true && audit.check !== false && audit.affected > 0) {
          return {
            kind: "success",
            text: `${text}\n\n⚠ ${audit.affected} legacy session(s) cannot be opened after the DSH session-format upgrade.\nOne-off repair: ${REPAIR_GUIDE_URL}`,
          };
        }
        return { kind: "success", text };
      },
