...
export function entryView(entry, reveal, connected) {
  return {
    id: entry.id,
    name: entry.name,
    tier: entry.tier,
    ...
    tools: Array.isArray(entry.tools)
      ? entry.tools.map((tool) => ({ name: tool.name, description: tool.description ?? "" }))
      : [],
    connected,
...
//   loadMcp(name, peek)                -> { ok, text, error? }
//   disconnectMcp(name)                -> { ok, error? }
export function mountUi(ctx, handlers, { dataDir, webServer }) {
  ...
  if (server !== undefined) {
    ...
    route("POST", "/skill-mcp-manager/mcp/load", async (body) => await handlers.loadMcp(body.name, body.peek === true));
    route("POST", "/skill-mcp-manager/mcp/disconnect", async (body) => await handlers.disconnectMcp(body.name));
  ...
  if (commands !== undefined) {
    ...
    disposers.push(commands.register({
      name: "mcp",
      description: "List every registered MCP server with tier, tool count and connection state",
      handler: async () => {
        const entries = await handlers.listMcp(false);
        const text = entries.length === 0
          ? "No MCP servers registered."
          : `Registered MCP servers (${entries.length}):\n${entries
              .map((entry) => `- ${entry.name} [${entry.tier}]${entry.connected ? " (connected)" : ""} — ${entry.toolCount} tool(s)`)
              .join("\n")}`;
        return { kind: "success", text };
      },
    }));
