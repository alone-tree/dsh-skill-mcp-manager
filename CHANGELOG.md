...
- **历史会话审核（只扫描提示，不修复）**：改掉写入格式只能保证"以后不再产生坏数据"，救不回已经落盘的旧会话。审核只读扫描 `~/.dsh/sessions` 里 v0 命名的日志，**由第一个 UI 请求触发**——启动路径里什么都不排：一次全量扫描本机 30–136 秒、单帧解压最长卡主线程 1.1 秒，而桌面端 host 的 `host-boot` 看门狗约 120 秒，插件的加载又发生在启动早期，所以 `await` 与 `setTimeout(8000)` **两种排法都落在窗口内、都会把 DSH 打进安全模式**（2026-09-14 两次实际发生）。以请求为触发点则天然安全：页面在问就说明 App 已经起来；`kick()` 每进程只跑一次，客户端在扫描进行期间轮询（答案落地即停）。检出受影响会话后在「设置 → 能力库」顶部显示提示、并弹浮层通知。浮层按状态分三种：**扫描中**报"正在检查、若历史会话较多请耐心等待、一次性动作"（可关闭，**关掉不中止检查**）；**有问题**报"有 N 个会话受影响"（横幅**没有关闭按钮**——误点不可能把真问题藏起来）；**没问题**报"没有会话受影响，请放心使用"，让这一次检查有始有终。**"结果"每个浏览会话只报一次**（记在浏览器会话存储里，不写宿主状态；退休后界面不留痕迹），否则每次打开页面都会重复报一次平安。**三种提示各用各的"已展示"标记，绝不共用**：横幅与浮层是两个各自持有状态的组件，共用同一个标记会互相抵消——先挂载的那个写下标记，另一个读到时就被判成"已经报过"，于是两边都不显示（2026-09-14 真机踩到：只有"正在检查"，结果与横幅都没有）；横幅的显示只由宿主状态决定。关闭写在浏览器会话存储里而不是组件内存里，所以"本次不再显示"关掉后刷新真的不会又冒出来。整个机制只有记录文件里的一个布尔 `check`：只有显式的 `true` 才扫；扫到受影响就保持为真并重新提示；扫到 0 就把它改假、以后不再扫，但仍报一次"没有受影响"——这一条同时让"已修好"和"从来没有过旧日志"的机器都不再付扫描成本。宿主明确回报**扫描中**与**已结束**两种状态；没有这个区分，"还在扫"和"永远不会再扫"在界面上一样，一台从没出现过旧日志的新装机就会被显示成"正在检查"并且永远等不到结果。schema-1 老记录按 `dismissedAt` **迁移**而不是丢弃：丢弃会让已经答复过的机器重启重扫，而一个重到会被杀死的扫描又写不完记录，就是死循环。只读，绝不写任何会话日志。**插件到提示为止：不附带修复脚本、不提供修复命令、不做自动修复**——修一批已落盘的会话是一次性、临时的活儿，为它往插件里塞可执行物会把一次性需求变成永久复杂度；修复由用户把报错交给 AI、由 AI 动手。测试：`test/session-audit-smoke.mjs`（含启动路径回归守卫、`running()` 三态断言、客户端契约与"各用各的标记"守卫、老记录迁移）。
