...
- **提示**：浮层按状态分三种——**扫描中**报"正在检查、可能要等一会儿、这是一次性动作"（可关闭；关掉**不中止**检查，结果出来照常再报一次）；**有问题**报"有 N 个会话受影响"并指向设置页；**没问题**报"没有会话受影响，请放心使用"。设置页横幅只对应"有问题"这一种（**没有关闭按钮**——误点不能把真问题藏起来）。客户端在扫描期间每 5 秒重查一次（上限 3 分钟），否则结果要等下次刷新才可见。
