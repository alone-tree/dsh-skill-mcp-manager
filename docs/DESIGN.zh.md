...
- **明文与 `$VAR` 引用都允许**：整段就是 `$NAME`（一个环境变量名）时，在启动本地工具的那一刻换成当时进程环境里的值；没有这个名字就换成空字符串，不把原文交出去。其余写法按字面值使用，包括 `$NAME` 后面还有别的内容、以及 `${NAME}`（某些 MCP 对变量引用不友好时可直接写值，避免引用写法本身带来新问题）。
- **本地工具不继承密钥和宿主内部变量**（2026-09-24）：stdio 子进程的继承范围与自带客户端 `scrubbedParentEnv` 一致。名字匹配 `KEY|PASSWORD|SECRET|TOKEN` 的变量，以及 `DSH_` 前缀（大小写不敏感），不交给子进程。条目里手写的 `env` 在过滤之后盖上去，所以同名项可以故意交出去。宿主能加载 `@deepseek-ai/dsh-subprocess` 时直接用它的函数（含代理变量的同一套处理）；加载不到时用同一条排除规则。远程地址的工具没有子进程环境，不走这条路。
...
- 时序：校验唯一性/命名/必填互斥 → **本地命令审批**（仅「新加了要执行的 stdio 命令」或「改了程序、参数、环境、工作目录」；在试连之前问，拒绝则不保存、不启动）→ **尝试加载**（试连 + `tools/list`，限时 30s）→ **成功**：写 registry（含快照）+ **同步写系统配置兜底实例**（§5.10）→ **失败**：返回报错给 AI，不写库、不写系统配置。
- **本地命令审批三档**（插件设置页，默认「跟随当前对话的 DSH 权限」，不写进安装配置）：完全权限（`danger-full-access`）直接放行；工作区可写和只读先经 `ctx.approval.request` 询问，人能看到程序、参数，以及之后预热和每个会话还会再启动它。另两档是每次都问、始终放行。宿主没有 `ctx.approval` 时保持静默放行，不假装问过。通道在但本会话策略拒绝时，不保存、不启动。改备注、只改档位、换远程地址、系统配置导入、管理页操作都不问。
...
- 快照保留服务器自报 `inputSchema`（与 `serverName` 等元信息一样来自服务器）：`mcp_load`（load/peek 两条路径）逐工具渲染参数结构，模型在调用前就能看到完整参数定义；`registry.json` 同步持久化。`mcp-catalog` 目录注入不受影响，仍只含名称+描述。
- 断开：**自动管理**——每个会话的连接保持到该会话结束/插件 dispose；**无空闲超时**。管理页不再提供「已连接」或「断开」。
- **常驻挂载不每回合拆装**（2026-09-24）：配置没变、并且这个会话里已经挂好时，`agent/pre-step` 什么也不做。关掉其中一个工具，或改了命令、参数、环境、工作目录、URL，必须马上反映到工具列表；连接身份变了就先关掉再连。某一轮没连上时，条目级记一笔失败（不写入注册表，进程重启会再试一次）：档位仍是常驻，管理页显示「连接失败，按按需处理」，还没挂上的会话不再自动启动，目录正文沿用现有 on-demand 行，让模型先 `mcp_load`。已经挂好的会话保持不动。管理页刷新快照，或对话里 `mcp_load` 成功，清掉失败并恢复自动挂载；还是失败就继续降级。按需连不上不降级。
...
3. **托管块外零残留**：托管块外被认领的 dsh-mcp-client 行从原位置移除（空的 `- insert:` 块一并清理）；未认领行（`importNativeMcp: false` 等）保持原样。非 MCP 内容 byte-for-byte 保留，夹在托管标记之间的非 MCP 行挪到结束标记之后。写前备份 `.bak-<timestamp>`，只留最近 5 份。
...
5. **卸载保障**（两个不同操作，勿混）：
   - **删除单个 MCP 条目**（UI"删除条目"）：整条从 `registry.json` 移除，托管块重生成后该行消失（补丁随注册表走）。该行 id 在本次进程内记为已删除，同一次 reconcile 不会把补丁里尚未重写的影子行再吸回来。
   - **卸载整个插件**：`/mcp prepare-uninstall` 与 `/prepare-uninstall` 做同一件事，把托管块整体替换为 HANDOVER 块——所有条目以**完整、启用**的 insert 写回（disabled 档保留 disabled），交还系统原生 dsh-mcp-client；原生无 on-demand 概念，交还后按 **eager** 加载，此为预期行为。`/mcp` 不带参数仍只列出服务器；后面写了认不出的话就提示用法，不静默列出。若没跑 prepare-uninstall 就直接卸载插件，托管行以 `disabled: true` 残留（配置完整），手动去除即可恢复。SKILL 无需卸载处理（递归 provider 随插件消失）。
   - 插件临时禁用（stop）不触发任何恢复。
...
> 背景：1.1.5 之前插件写自定义 `source.kind`，DSH 2.0.9 的 v0→v3 迁移边用封闭集合审计 message source，导致旧会话整体打不开（2026-09-11 事故）。改掉写入格式只能保证"以后不再产生坏数据"，救不回已经落盘的。
>
> **本插件只负责告警，不负责修复。** 扫出受影响的会话后，插件做的唯一一件事是把问题摆到用户面前；修复由用户把报错交给 AI、由 AI 动手完成。这不只是取舍，是刻意的边界：修一批已经落盘的会话是一次性、临时的活儿，为它往插件里塞可执行物（修复脚本、修复命令、自动修复开关）会把一个一次性需求变成永久复杂度，而插件本身要尽量简单：插件里没有任何"一键修复"入口，装插件也不得到任何修复脚本。
>
> 修复脚本只存在于**仓库**里（`tools/repair-legacy-sessions.mjs`），由 issue #2 指向它——**一个来源、一条路径**：要修的人从 GitHub 取那一份原文，npm 包里没有它（`.npmignore` 排除）。
...
4. **扫描（重活，只读）。** 找 v0 命名的日志，按帧切分后同步逐帧解压找 `"kind":"mcp-catalog"`；文件之间每 100 ms 让出事件循环；**绝不写任何会话日志**。因为要占着事件循环解码，**扫描期间历史会话目录可能暂时打不开**——等待提示里明说了这一点，免得用户以为坏了。本机实测冷扫 577 份 104.4 秒，命中早退约 3.6 秒。
5. **落记录 + 告知（这一轮检查的收尾）。** 结果写进 `session-audit.json`（tmp + rename），**只有本次真的扫过才告知一次**：`affected > 0` → `check` 保持 `true`，浮层报「有 N 个会话受影响」，设置页横幅展开完整说明（报错原文 + 交给 AI），提醒一直保留到问题被处理；`affected === 0` → 浮层报「没有会话受影响，请放心使用」，同时该检查就此结束（`check` 落成 `false`），以后不再扫描。
6. **AI 修复（插件完全不参与）。** 用户把提示里的报错原文交给 AI，AI 去读那些日志、搞清帧结构与坏 source 形状、动手改并自行验证。插件做的一切只是把这件事摆到台面上，没有第二条修复路径。
7. **收敛（整件事的收尾）。** 用户拿去修 → 修好后下次启动走一遍阶段 5 的"没有受影响"分支，事情就结束了。用户决定不修 → 把 `"check"` 手动写成 `false` 即停。**退休后界面不留任何痕迹**（不会隔三差五报一次平安），而「没有会话受影响」只在检查刚结束那一次出现——这两件事说的是同一件事，只是分别对"之后每次打开"和"检查当次"而言。

一句话：**插件负责"发现并告诉你"，AI 负责"修"，布尔 `check` 负责"修完之后闭嘴"。** 全流程只写一个文件（插件自己的 `session-audit.json`），对会话日志只读。

- **触发**：**由第一个 UI 请求触发**（`getSessionAudit` 里 `void auditRunner.kick()`），**启动路径里什么都不排**。这是唯一安全的位置：一次全量扫描本机 30–136 秒，且**单帧解压最长卡主线程 1.1 秒**，而桌面端 host 对 `host-boot` 有约 **120 秒看门狗**，超时即进安全模式。插件在启动早期就加载完，所以"`await` 进 `apply()`"和"`setTimeout(8000)`"**都落在那个窗口里**——2026-09-14 两种都试过，两次都把 DSH 打进安全模式。页面在问就说明 App 已经起来，因此以请求为触发点；`kick()` 每个进程只跑一次，客户端在 `running()` 为真期间轮询（答案落地那一刻就停，不会对着已结束的扫描空等）。**回归守卫**：`test/session-audit-smoke.mjs` 第 8 节断言 `apply()` 返回后 10 秒内**不得有任何记录出现**，然后 `kick()` 才必须产出记录、并在扫描在跑期间 `running()` 为真、结束后为假——"有没有在启动路径里排扫描"和"界面该不该显示正在检查"对其它测试完全不可见。
...
- **唯一机制是一个布尔 `check`**，存在 `~/.dsh/skill-mcp-manager/session-audit.json`：
  ...
  - `check` 为真 → 扫描；`affected > 0` 则保持为真并在**每次启动**重新提示；`affected === 0` 则**把它改假、此后不再扫描**，同时仍报一次"没有会话受影响"（阶段 5）。
  - 最后一条同时解决两件事：已修好的机器不必每次启动白等（干净的大库无法提前退出，扫得更慢），以及从来没出现过旧日志的新装机根本不会被打扰。
- **提示**：浮层按状态分三种——**扫描中**报"正在检查、可能要等一会儿、这是一次性动作"（可关闭；关掉**不中止**检查，结果出来照常再报一次）；**有问题**报"有 N 个会话受影响"并指向设置页；**没问题**报"没有会话受影响，请放心使用"。设置页横幅只对应"有问题"这一种。客户端在扫描期间每 5 秒重查一次（上限 3 分钟），否则结果要等下次刷新才可见。
  - **"在扫"只指扫描本身**：宿主读记录、判断要不要扫的那几毫秒不算在扫——把它算进去，一个永远不会再扫的检查也会闪一次"正在检查"（2026-09-14 真机踩到）。`running()` 因此从扫描真正开始那一刻才为真。
  - **横幅的显示只由宿主状态（`check` + `affected`）决定**，与浮层的任何标记无关，也不在浏览器里记任何"关过"的状态——它随页面回来，只在问题被处理掉时消失。它有 × 可关，但那只作用于这一次挂载。
  - **每种提示各用各的标记，绝不共用**：横幅与浮层是两个各自持有状态的组件，共用同一个"已展示"标记会互相抵消——先挂载的那个写下标记，另一个读到时就被判定"已经报过"，于是**两边都不显示**（2026-09-14 真机踩到：只看到"正在检查"，之后结果和横幅都没有）。
  - 浮层的关闭写在浏览器会话存储里（不是组件内存里），所以关掉之后**刷新页面不会又冒出来**——"本次不再显示"要真的算数。
  - **"结果"只在"本次进程里真的扫过"之后报一次**：退休之后重新打开页面，读到的只是一个**早就存在的结论**，不该当成新闻再报一遍。宿主因此回报"本次检查到底跑没跑"（`checked`），客户端据此决定要不要报结果。不能靠浏览器里记标记来挡这件事——浏览器每次重启都是全新的会话存储，**"已报过"跨不过一次重启**（2026-09-14 真机踩到：修好之后每次重启仍弹"正在检查 + 没有会话受影响"）。
- **兜底**：如果用户决定不修，把记录里的 `"check"` 改成 `false` 即停；这一条写在提示与 issue 里。
- **决策：只扫描提示，不做修复**。扫描是插件该干的（判定成本低、结论稳定），修复不是（一次性的数据手术、每个库的形态都不同）。所以插件不附带修复脚本、不提供修复命令、不做自动修复；用户拿到报错后交给 AI 处理。同理，其它出口（如 `/mcp` 命令输出）不重复播报这条告警——提示只在设置页横幅与浮层各出现一次。
- **测试**：`test/session-audit-smoke.mjs`（候选筛选、三种形状、正文转义提及不误判、已修形态判干净、明文日志、受影响保持重扫、扫到 0 退休、退休后不重写记录、空库立即退休、缺失根目录不报错、`running()` 只在扫描真在跑时为真、客户端契约与"三种提示各用各的标记、横幅不读浮层标记"的字符串守卫）。
...
- **浏览器请求的地址检查**（2026-09-24）：带 `Origin` 的读和写都必须同源，并且主机名是 `localhost`、本机名或 IP 字面量。局域网主机名放在插件设置的允许名单里，默认空；不在名单里的主机名打不开这些接口，也就不能自己把自己加进去。不带 `Origin` 的调用保持原样（POST 仍要同源，GET 不额外拦截）。设置页同一处还有「模型登记本地程序」三档。常驻连接失败时，列表在档位旁边显示「连接失败，按按需处理」。
