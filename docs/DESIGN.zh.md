...
- **提示**：浮层按状态分三种——**扫描中**报"正在检查、可能要等一会儿、这是一次性动作"（可关闭；关掉**不中止**检查，结果出来照常再报一次）；**有问题**报"有 N 个会话受影响"并指向设置页（关闭只作用于本次会话）；**没问题**报"没有会话受影响，请放心使用"。设置页横幅只对应"有问题"这一种（**没有关闭按钮**——误点不能把真问题藏起来）。**"结果"这一类每个浏览会话只报一次**（记录在浏览器会话存储里，不写宿主的任何状态；退休后界面不留任何痕迹），否则"报平安"会在每次打开页面时重复出现。客户端在扫描期间每 5 秒重查一次（上限 3 分钟），否则结果要等下次刷新才可见。
...
- **测试**：`test/session-audit-smoke.mjs`（候选筛选、三种形状、正文转义提及不误判、已修形态判干净、明文日志、受影响保持重扫、扫到 0 退休、退休后不重写记录、空库立即退休、缺失根目录不报错、`running()` 只在扫描真在跑时为真、客户端契约与"结果只报一次"的字符串守卫）。
