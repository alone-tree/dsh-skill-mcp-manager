...
2. **第一个 UI 请求触发（唯一触发点）。** 浏览器端 `useLegacyAudit()` 请求 `/skill-mcp-manager/session-audit`；handler 做三件事：`void auditRunner.kick()`（触发扫描但**不等它**，因为这个响应在页面加载路径上）、读 `session-audit.json`、没有记录则返回 `{pending:true}`，客户端每 5 秒重问（上限 3 分钟）。`kick()` 每进程只跑一次；记录已存在时不阻塞。
