...
- **触发**：**延后 8 秒**（`SESSION_AUDIT_DELAY_MS`，`.unref()` + `ctx.effect` 清理）——**绝不能在启动路径里 `await`**。一次全量扫描在本机是 46–136 秒，而桌面端 host 对 `host-boot` 阶段有约 **120 秒看门狗**；把它 `await` 进 `apply()` 会让每次重启都超过看门狗、把 DSH 直接打进安全模式（2026-09-14 实际发生）。延后之后启动零成本：客户端本来就在 `pending` 期间轮询，而从第二次启动起记录已在磁盘上，提示在页面打开时就已经在了。**回归守卫**：`test/session-audit-smoke.mjs` 第 8 节断言 `apply()` 返回时记录尚不存在、但它随后一定出现——"有没有被 await" 对其它测试完全不可见。
