...
>
> **本插件只负责告警，不负责修复。** 扫出受影响的会话后，插件做的唯一一件事是把问题摆到用户面前；修复由用户把报错交给 AI、由 AI 动手完成。这不只是取舍，是刻意的边界：修一批已经落盘的会话是一次性、临时的活儿，为它往插件里塞可执行物（修复脚本、修复命令、自动修复开关）会把一个一次性需求变成永久复杂度，而插件本身要尽量简单。这也是为什么这里既没有 `tools/` 下的修复脚本，也没有任何"一键修复"入口。
