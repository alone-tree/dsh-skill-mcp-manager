...
2. **第一个 UI 请求触发（唯一触发点）。** 浏览器端 `useLegacyAudit()` 请求 `/skill-mcp-manager/session-audit`；handler 先 `auditRunner.kick()`（触发扫描但**不等它**，因为这个响应在页面加载路径上），再按**扫描是否真的在跑**作答：在跑 → `{scanning:true}`，客户端每 5 秒重问（上限 3 分钟）并显示「正在检查」；已经不用再扫（退休过、或本次已出结果）→ `{done:true, …}`，界面保持安静。这一区分是必须的：没有它，一台**永远不会再扫**的机器（从来没出现过旧日志的新装机、或已修好的机器）会被显示成"正在检查"并且永远等不到结果。`kick()` 每进程只跑一次。
...
5. **落记录 + 告知（这一轮检查的收尾）。** 结果写进 `session-audit.json`（tmp + rename），**这一轮不管结果如何都告知一次**：`affected > 0` → `check` 保持 `true`，浮层报「有 N 个会话受影响」，设置页横幅展开完整说明（报错原文 + 交给 AI），提醒一直保留到问题被处理；`affected === 0` → 浮层报「没有会话受影响，请放心使用」，同时该检查就此结束（`check` 落成 `false`），以后不再扫描。
...
- **提示**：浮层按状态分三种——**扫描中**报"正在检查、可能要等一会儿、这是一次性动作"（可关闭；关掉**不中止**检查，结果出来照常再报一次）；**有问题**报"有 N 个会话受影响"并指向设置页；**没问题**报"没有会话受影响，请放心使用"。设置页横幅只对应"有问题"这一种（**没有关闭按钮**——误点不能把真问题藏起来）。**"结果"这一类每个浏览会话只报一次**（记录在浏览器会话存储里，不写宿主的任何状态；退休后界面不留任何痕迹），否则"报平安"会在每次打开页面时重复出现。客户端在扫描期间每 5 秒重查一次（上限 3 分钟），否则结果要等下次刷新才可见。
  - **每种提示各用各的标记，绝不共用**：横幅与浮层是两个各自持有状态的组件，共用同一个"已展示"标记会互相抵消——先挂载的那个写下标记，另一个读到时就被判定"已经报过"，于是**两边都不显示**（2026-09-14 真机踩到：只看到"正在检查"，之后结果和横幅都没有）。横幅的显示只由宿主状态（`check` + `affected`）决定，与浮层的标记无关。
  - 关闭写在浏览器会话存储里（不是组件内存里），所以关掉之后**刷新页面不会又冒出来**——"本次不再显示"要真的算数。
