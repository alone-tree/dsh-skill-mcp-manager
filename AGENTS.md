...
- **启动看门狗约 120 秒：`apply()` 里不能 `await` 慢操作**（2026-09-14 实际把 DSH 打进安全模式）：桌面端 host 对 `host-boot` 阶段有约 123 秒看门狗（实测 `lifecycle-events/startup.jsonl` 的 `stage.failed durationMs=123058`），超时即判失败进安全模式。本插件把一次 46–136 秒的历史会话审计 `await` 进启动路径后就变成"关了就打不开"，而 host 日志里**连 `skill-mcp-manager: ready` 都没有**——这正是排查时的关键信号。规则：启动路径只做必要且快的事，任何重活（扫描、遍历、批量 IO）都必须 `setTimeout(...).unref()` 延后或交给后台。同类坑还有 2026-09-11 的"单个插件加载失败 → 整棵插件树失败 → 安全模式"。
