...
- **MCP tab** —— 所有已注册服务器（档位徽标、工具数、连接状态）：切档位、查详情（命令 / env / headers / 工具）、逐工具“启用/禁用”（黑名单，新工具默认启用）、密钥打码/显示、加载/查看描述/断开、删除条目。禁用工具不向 AI 暴露，并在实际调用入口强制拒绝。顶部设置行可调目录的**工具描述截断长度**（默认 150 字符）。
